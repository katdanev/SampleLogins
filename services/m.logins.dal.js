...
const dal = require("./m.auth_db.js"); // connect to the database (mongoDB)
...
//  let newLogin = JSON.parse(`{ "username": "` + username + `", "password": "` + password + `" }`);
 ...
 , "email": "${email}", "uuid": "${uuid}" }`);
  ...
  // Цю частину ми можемо використати для вставки даних в базу даних на початку тестування ми можемо її закоментувати
  try {
    await dal.connect();
